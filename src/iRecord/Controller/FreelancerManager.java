...
public class FreelancerManager {
    ...
    /**
     * this method connects a freelancer to studio and adds rating to the
     * freelancer might need remove method as well
     */
    ...
    /**
     * This method returns the dates and places freelancer is scheduled for a
     * session
     *
     * @return
     */
    ...
    /**
     * =========================================================== public class
     * InsertPicture { public static void main(String[] args) throws Exception,
     * IOException { Class.forName("sun.jdbc.odbc.JdbcOdbcDriver"); String
     * url="jdbc:odbc:MyDsn"; Connection conn=DriverManager.getConnection(url);
     * String INSERT_PICTURE = "insert into Img(ID,Image) values (?, ?)";
     *
     * FileInputStream fis = null; PreparedStatement ps = null; try {
     * conn.setAutoCommit(false); File file = new File("laura.jpg"); fis = new
     * FileInputStream(file); ps = conn.prepareStatement(INSERT_PICTURE);
     * ps.setString(1, "001"); ps.setBinaryStream(2, fis, (int) file.length());
     * ps.executeUpdate(); conn.commit(); } finally { ps.close(); fis.close(); }
     * } }
     *
     *
     */
